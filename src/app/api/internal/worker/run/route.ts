/**
 * POST /api/internal/worker/run - Cron endpoint for sticker generation worker
 * 
 * This endpoint is triggered by Vercel Cron or an external scheduler every 1 minute.
 * It processes pending sticker generation jobs and cleans up zombie jobs.
 * 
 * Authentication: Requires WORKER_SECRET environment variable
 */

import { NextRequest, NextResponse } from 'next/server'
import { runWorker, WorkerRunResult } from '@/lib/worker/sticker-worker'
 ...
 * Trigger the sticker generation worker
 */
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    const result: WorkerRunResult = await runWorker()
