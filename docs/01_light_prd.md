...
**跨用户内容去重**（v2.0）：
* 相同PDF内容的自动贴纸跨用户共享缓存,降低成本、提升速度
* 缓存基于PDF内容哈希而非用户身份,首次生成后其他用户立即可用
* 用户数据隐私：仅共享AI生成的讲解内容,不共享用户上传的PDF或个人学习数据
...
* ✅ 跨用户缓存隐私：自动贴纸跨用户共享仅指AI生成内容,不涉及用户身份、PDF内容或学习行为
...
* **退款机制**：系统检测到生成失败(AI错误、超时等)后,自动将配额退还至用户账户
