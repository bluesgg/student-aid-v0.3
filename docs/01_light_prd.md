...
**三栏布局**（左中右）：
* **左侧**：PDF阅读器(分页/缩放/记忆lastReadPage) - 40-50%宽度
* **中栏**：贴纸栏(自动讲解 + 选中讲解) - 25-30%宽度
* **右栏**：问答与总结区 - 25-30%宽度

**布局调整**：用户可拖拽分隔条手动调整三栏宽度，偏好保存在localStorage，不占用配额

**优势**：贴纸和问答可同时可见，无需上下滚动切换；视觉对比更清晰；工作流更顺畅（阅读PDF→查看贴纸→提问，视线自然从左到右流动）
...
* 所有配额按用户注册日期周期重置(例如9月7号注册,则每月7号00:00 UTC重置;需Cron Job实现,见04_TECH §13.1)
...
* 前端提示："本月配额已用尽(X/Y),将于M月D日重置。"(D为注册日)
