...
| 能力 | 触发方式 | 计费规则 |
|------|---------|---------|
| 自动讲解 | 点击"Explain this page" | 300次/账户/天(不计入配额) |
...
| 配额桶 | 包含操作 | 限制 |
|--------|---------|------|
| learningInteractions | 选中讲解 + 问答 | 150次/账户 |
| documentSummary | 文档总结 | 100次/账户 |
| sectionSummary | 章节总结 | 65次/账户 |
| courseSummary | 课程级提纲 | 15次/账户 |
...
| 维度 | 粒度 | 限制 | 重置时间 |
|------|------|------|---------|
| 按账户每日 | (userId,date_UTC) | 300次/天 | 每日00:00 UTC |

**触达行为**：
* 返回HTTP 429 + `AUTO_EXPLAIN_LIMIT_REACHED`(错误码见03_API §6)
* 前端提示倒计时："今日自动讲解已用完(300/300),将在X小时后重置。您仍可使用选中文本讲解。"
